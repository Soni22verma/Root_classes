...
//       // Handle the response structure where attempted is directly in data
//       if (res.data && res.data.attempted === true) {
//         return {
//           attempted: true,
...
//       // Also handle success wrapper if present
//       if (res.data && res.data.success && res.data.data && res.data.data.attempted === true) {
//         return {
//           attempted: true,
...
//         if (errorData.attempted === true) {
//           return { attempted: true, result: errorData.result || errorData.data || null };
//         }
//         if (errorData.message && (errorData.message.includes('already') || errorData.message.includes('completed') || errorData.message.includes('attempted'))) {
//           return { attempted: true, result: errorData.data || null };
//         }
//       }
//       return { attempted: false, result: null };
//     }
...
//       studentId: studentId,
//       testId: testId,
//       answers: studentAnswers  // ✅ This is already an object with questionId as key
//     };
...
//     if (test.isCompleted) {
//       toast.error('You have already completed this test. You cannot retake it.');
//       return;
//     }
...
//       toast.error('You have already attempted this test. You cannot retake it.');
//       await GetAllPublishedTest();
//       return;
//     }
...
//     // Format answers properly with the correct questionId
//     const formattedAnswers = {};
    
//     // Log answers for debugging
//     console.log('Student answers:', answers);
//     console.log('Questions with correct answers:', questions.map(q => ({
//       questionId: q.questionId,
//       correctAnswer: q.correctAnswer,
//       studentAnswer: answers[q.id]
//     })));
    
//     // Calculate percentage locally for UI feedback
//     let correctCount = 0;
//     questions.forEach(question => {
//       const userAnswer = answers[question.id];
//       if (userAnswer !== undefined && userAnswer === question.correctAnswer) {
//         correctCount++;
//         console.log(`Question ${question.questionId}: Correct! Answer: ${userAnswer}`);
//       } else if (userAnswer !== undefined) {
//         console.log(`Question ${question.questionId}: Wrong. Student: ${userAnswer}, Correct: ${question.correctAnswer}`);
//       }
//     });
    
//     const percentage = (correctCount / questions.length) * 100;
//     setScore(percentage);
//     console.log(`Local calculation: ${correctCount}/${questions.length} correct = ${percentage}%`);
...
//         console.log('Server response score:', result.data);
//       }
//     } else {
//       setTestCompleted(true);
//       setTimerActive(false);
...
//     console.log(`Selected answer for ${questionId}: ${answerIndex}`);
//     setAnswers(prev => ({
//       ...prev,
...
const StudentTestPanel = () => {
  ...
  const checkAttemptTest = async (testId) => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        // If the API returns that the test has been attempted
        return {
          attempted: true,
      ...
      console.log('Error checking attempt:', error);
      // If error response indicates test already attempted
  ...
  const GetAllPublishedTest = async () => {
    try {
      ...
      if (res.data && res.data.success && res.data.data) {
        // Fetch completed tests from both APIs
        const completed = await fetchCompletedTests();
        const completedTestIds = new Set(completed.map(t => t.testId));
        
        // Also check each test individually using checkAttemptTest
        const testsWithAttemptStatus = await Promise.all(
          res.data.data.map(async (test) => {
            const attemptStatus = await checkAttemptTest(test._id);
            return {
  ...
  const SubmitTest = async (testId, studentAnswers) => {
    try {
      ...
      if (res.data && res.data.success) {
        setScoreDetails(res.data.data);
        // Refresh the test list to mark this test as completed
        await GetAllPublishedTest();
        return {
          success: true,
          data: res.data.data
      ...
      if (error.response) {
        ...
        // If the error indicates test already submitted
        if (errorMessage.includes('already') || errorMessage.includes('completed')) {
          await GetAllPublishedTest();
          handleBackToTests();
        }
  ...
  const handleStartTest = async (test) => {
    // First check if test is already completed from our state
    if (test.isCompleted) {
    ...
    // Double check with API before starting
    setCheckingAttempt(true);
    const attemptStatus = await checkAttemptTest(test.id);
    setCheckingAttempt(false);
    ...
    if (attemptStatus.attempted) {
      ...
      // Refresh the test list to update the UI
      await GetAllPublishedTest();
      return;
    }
    
    // Additional check: verify in local completed tests
