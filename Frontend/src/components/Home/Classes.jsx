    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-7 grid grid-rows-2 gap-4">
          ...
          {/* Card 02 — dark blue */}
          <div className="bg-[#0a1628] rounded-2xl p-7 flex flex-col justify-between group bg-dot-dark relative overflow-hidden">
            {/* Blue glow accent */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#0078FF]/20 rounded-full blur-3xl pointer-events-none" />
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-blue-500/20 text-[#60b4ff]">{programs[1].tag}</span>
                <span className="text-6xl font-black text-white/5 leading-none">{programs[1].num}</span>
              </div>
              <h3 className="text-2xl font-bold text-white mb-2">{programs[1].title}</h3>
              <p className="text-sm text-blue-200/70 leading-relaxed">{programs[1].desc}</p>
            </div>
            <div className="flex items-center justify-between mt-6">
              <div className="flex gap-2 flex-wrap">
                {programs[1].courses.map((_, i) => (
                  <span key={i} className="text-xs text-blue-300/60 bg-white/5 px-3 py-1 rounded-full">Option {i + 1}</span>
                ))}
              </div>
              <Link to="/course">
                <button className="flex-shrink-0 ml-3 px-4 py-2 rounded-xl text-sm font-semibold bg-[#0078FF] text-white hover:bg-[#0066DD] transition shadow-sm shadow-blue-900">
                  Enroll →
                </button>
              </Link>
            </div>
          ...
          <div className="bg-white rounded-2xl p-7 flex flex-col justify-between border border-gray-200 hover:border-[#FB0500]/40 hover:shadow-md transition-all group">
            ...
            <div className="flex items-center justify-between mt-6">
              ...
              <Link to="/course">
                <button className="px-4 py-2 rounded-xl text-sm font-semibold bg-[#0a0a0a] text-white hover:bg-[#FB0500] transition-colors">
                  Start Learning →
                </button>
              </Link>
            </div>
